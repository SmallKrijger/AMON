...
def NOMAD_execution2(param_file_name, nb_wt, X0):
    ...
    plot_f.plot_terrain(result['x_best'][0::2], result['x_best'][1::2], "EAP", "GWh", obj_function_value, nb_wt, ub, boundary_shapely, exclusion_zones_shapely, max_index=max_index, max_ws=max_ws, cg=cg, plot_flow_map=False, save=True, save_name="instances_results/" + instance_number + "/layout_" + i + ".png")
    ...
    f.write(str(nb_wt) + " " + str(obj_function_value) + " " + str(wl) + " " + str(t_Nomad) + "\n")
    # f.write(str(nb_wt) + " " + str(0) + " " + str(0) + " " + str(0) + "\n")
    f.close() 

if __name__ == '__main__':
    for i in range(70, 85, 5):
        X0 = init_sol.initial_sol_test("instances/1/param.txt", i)
        NOMAD_execution2("instances/1/param.txt", i, X0)